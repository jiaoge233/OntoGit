...
- `xiaogugit/docker-compose.yml` 默认只把宿主机 `127.0.0.1:${XG_PORT:-8000}` 映射到容器 `8000`。
- 这样外部网络不能直接访问 `:8000`，业务入口统一走 gateway 的 `:8080`。
- gateway 容器通过 `host.docker.internal:8000` 访问 xiaogugit。
- 如确需临时外部调试，可显式设置 `XG_BIND_ADDR=0.0.0.0`，但生产不建议这么做。
