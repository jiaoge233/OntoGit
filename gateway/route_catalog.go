...
func gatewayRouteCatalog() []RouteDoc {
	return []RouteDoc{
		...
		{
			Name:        "gateway_users_page",
			Method:      "GET",
			Path:        "/ui-users",
			Module:      "gateway",
			Auth:        "browser session or bearer",
			Description: "Gateway user registration and API key management page.",
		},
		{
			Name:        "gateway_api_docs_page",
			Method:      "GET",
			Path:        "/ui-api-docs",
			Module:      "gateway",
			Auth:        "browser session or bearer",
			Description: "Gateway-owned API catalog page. /docs redirects here to avoid mixed upstream docs.",
		},
		{
		...
		{
			Name:        "gateway_user_register",
			Method:      "POST",
			Path:        "/api/users/register",
			Module:      "gateway",
			Auth:        "none",
			Description: "Register a gateway user, issue a short-lived user token, and create an initial API key.",
		},
		{
			Name:        "gateway_user_login",
			Method:      "POST",
			Path:        "/api/users/login",
			Module:      "gateway",
			Auth:        "none",
			Description: "Login with gateway username/password and return a user bearer token.",
		},
		{
			Name:        "gateway_user_api_keys_list",
			Method:      "GET",
			Path:        "/api/users/api-keys",
			Module:      "gateway",
			Auth:        "user bearer or user X-API-Key",
			Description: "List current user's API keys without exposing raw key values.",
		},
		{
			Name:        "gateway_user_api_keys_create",
			Method:      "POST",
			Path:        "/api/users/api-keys",
			Module:      "gateway",
			Auth:        "user bearer or user X-API-Key",
			Description: "Create a new API key for the current user. The raw key is returned once.",
		},
		{
			Name:        "gateway_user_api_keys_revoke",
			Method:      "DELETE",
			Path:        "/api/users/api-keys/{id}",
			Module:      "gateway",
			Auth:        "user bearer or user X-API-Key",
			Description: "Revoke one API key owned by the current user.",
		},
		{
			Name:        "gateway_version_star",
			Method:      "POST",
			Path:        "/api/stars/star",
			Module:      "gateway",
			Auth:        "browser session, bearer, or X-API-Key",
			Description: "Idempotently star one ontology version. MySQL stores one vote per caller and gateway forwards to xiaogugit only when the vote state changes.",
		},
		{
			Name:        "gateway_version_unstar",
			Method:      "POST",
			Path:        "/api/stars/unstar",
			Module:      "gateway",
			Auth:        "browser session, bearer, or X-API-Key",
			Description: "Idempotently remove the current caller's star from one ontology version.",
		},
		{
